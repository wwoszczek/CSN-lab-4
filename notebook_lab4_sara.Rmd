...
for (language in languages) {
  ...
  # is used in statistics to describe the process of creating a smooth curve that matches the data points in a scatterplot.
  lines(lowess(variance_df$Group.1, variance_df$d), col="red")
  #check if, by using the aggregate, the data is homoscedastic.
  #variance_df_mean = aggregate(mean_df, list(mean_df$vertex), var)
  #variance_df_mean[is.na(variance_df_mean)] <- 0
  #plot(variance_df_mean$Group.1, variance_df_mean$d, xlab = "N. Vertex", ylab = "Variance of Mean", main = paste(language, " variance plot"))
...
#}
  ...
  results_1_plus <- non_linear_models_fitting_and_plotting(mean_df, initial_values_1_plus, model_formula = "((0.5) * vertex) ^ b + q", model_name="Model 1+", max_iter = 1000,  algorithm = "port", lower = -2, upper = 10.0)
...
results_2_plus <- non_linear_models_fitting_and_plotting(mean_df, initial_values_2_plus, model_formula = "a*(vertex^b) + q", model_name="Model 2+ - Power Law Model ", algorithm = "port", lower = -2, upper = 10.0)
...
#NOTE: Turkish language parameter 'a' is 10, so maybe we could set a higher upper bound.
