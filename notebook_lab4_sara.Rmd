...
for (language in languages) {
  ...
  #check by visual inspection 
    plot(variance_df$Group.1, variance_df$d, xlab = "Vertex", ylab = "Residuals", main = language)

    
  # par(mfrow = c(2, 2))  # Set up a multi-plot layout
  # 
  # for (i in 1:length(model_list)) {
  #   model <- model_list[[i]]
  #   residuals <- residuals_list[[i]]
  #   plot(predict(model), residuals, main = paste(language, " - Model ", i), xlab = "Predicted Values", ylab = "Residuals")
  # }
  # 
  # 
  # 
...
for (language in languages) {
  ...
  results_5 <- c()
  if (language == "Czech"){
      #initial_values_5 = list(a =  2.76187e+92, b = 7.255974, c = -0.0504282) 
      results_5 <- non_linear_models_fitting_and_plotting(mean_df, initial_values_5, model_formula = "a * (vertex^b) *exp(c*vertex)", model_name=" Model 5 - Generalized Power Law Model", max_iter = 1000, algorithm = "plinear")
  }
  else{
  ...
  initial_values_1_plus <- list(b = results_1$coeff, d=0)
  results_1_plus <- non_linear_models_fitting_and_plotting(mean_df, initial_values_1_plus, model_formula = "((0.5) * vertex) ^ b + d", model_name="Model 1+", max_iter = 1000,  algorithm = "port", lower = 0.0, upper = 1)

 
  ################ Model 2+   #################
#initial_values_2_plus <- list(a =  exp(coef(lm2)[1]), b = coef(lm2)[2], d = 0)
  initial_values_2_plus <- list(a = results_2$coeff[1], b =  results_2$coeff[2], d = 0)
results_2_plus <- non_linear_models_fitting_and_plotting(mean_df, initial_values_2_plus, model_formula = "a*(vertex^b) + d", model_name="Model 2+ - Power Law Model ", algorithm = "port", lower = 0.0, upper = 1.0)

  ################ Model 3+  #################
  #initial_values_3_plus <- list(a =  exp(coef(lm3)[1]), c = coef(lm3)[2], d =0)
initial_values_3_plus <- list(a = results_3$coeff[1], c = results_3$coeff[2], d =0)
  results_3_plus <- non_linear_models_fitting_and_plotting(mean_df, initial_values_3_plus, model_formula = "a*(exp(vertex*c)) + d", model_name="Model 3+ - Exponential Model", algorithm = "port",  lower = -10, upper = 10)

  ################ Model 4+  #################
  #initial_values_4_plus<-list(a = coef(lm4)[1], d = 0)
  initial_values_4_plus<-list(a = results_4$coeff, d = 0)
  results_4_plus <- non_linear_models_fitting_and_plotting(mean_df, initial_values_4_plus, model_formula = "a*log(vertex) + d", model_name=" Model 4+ - Logarithmic Model", algorithm = "port", lower = -10, upper = 10)
        
  
  ################ Model 5+  #################
  initial_values_5_plus = list(a=results_5$coeff[1],
                    b=results_5$coeff[2],
                    c =results_5$coeff[3], d=0)
  results_5_plus <- non_linear_models_fitting_and_plotting(mean_df, initial_values_5_plus, model_formula = "a * (vertex^b) *exp(c*vertex) + d", model_name=" Model 5+ - Generalized Power Law Model", algorithm = "port", lower = list(-10, -10, -10, 0), upper = 10)
